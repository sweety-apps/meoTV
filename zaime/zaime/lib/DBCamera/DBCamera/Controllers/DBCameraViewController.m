...
- (void) captureImageDidFinish:(UIImage *)image withMetadata:(NSDictionary *)metadata
{
    ...
    } else
    {
        ...
        BACK(^{
            UIImage *dest = [UIImage croppedImageWithImage:image :CGRectMake(0, (image.size.height-image.size.width)/2.f+50, image.size.width, image.size.width)];
            MAIN((^{
                DBCameraSegueViewController *segue = [[DBCameraSegueViewController alloc] initWithImage:dest thumb:[UIImage returnImage:dest withSize:(CGSize){ newW, newH }]];
