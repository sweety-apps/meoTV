...
- (id)initWithImages:(NSArray *)images degress:(NSMutableArray*)degress frame:(CGRect)frame;
{
    ...
    if(self)
    {
        ...
        _roteDegress            = [degress copy];
        _borderWidth            = 0.0f;
        _imageViews             = [[NSMutableArray alloc] init];
        _animationType          = WCGalleryAnimationFade;
        _stackRadiusOffset      = 10.0f;
...
- (void)setSelectAction:(SelectAblumAction)action
{
    select = action;
}
